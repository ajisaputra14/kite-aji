...
function displayBanner() {
  ...
  const banner = figlet.textSync('\n AJIE S', { font: "ANSI Shadow", horizontalLayout: 'Speed' });
  banner.split('\n').forEach(line => {
    console.log(chalk.cyanBright(line.padStart(line.length + Math.floor((width - line.length) / 2))));
  });
  console.log(chalk.cyanBright(' '.repeat((width - 43) / 2) + '=== Github : https://github.com/ajisaputra14 ==='));
  console.log(chalk.yellowBright(' '.repeat((width - 39) / 2) + '++ KITE AI AUTO CHAT BOT WITH MANY FEATURE ++\n'));
}
...
async function dynamicDelay(min = 20000, max = 40000) {
  const delay = Math.floor(Math.random() * (max - min + 1)) + min;
  let remaining = delay / 1000;

  console.log(chalk.cyan(` ┊ ⏳ Delay selama: ${remaining} detik`));
  const countdown = setInterval(() => {
    remaining--;
    process.stdout.write(chalk.cyan(` ┊ ${remaining} detik sebelum melanjutkan\r`));
    if (remaining <= 0) {
